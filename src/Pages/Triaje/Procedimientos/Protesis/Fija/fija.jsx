...
const ProtesisFija = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      formData.matriculaEstudiante_protesis_fija.trim() === "" ||
      formData.nombreEstudiante_protesis_fija.trim() === "" ||
      formData.apellidoEstudiante_protesis_fija.trim() === ""
    ) {
    ...
    if (
      !nameRegex.test(formData.nombreEstudiante_protesis_fija) ||
      !nameRegex.test(formData.apellidoEstudiante_protesis_fija)
    ) {
    ...
    fetch(
      "https://3lmv2y6pmb.execute-api.us-east-1.amazonaws.com/development/dentist-appointment-update-request",
      {
        ...
        body: JSON.stringify({
          ...formData,
          pageType: "protesis_fija", // Change to appropriate page type based on the page
        }),
      }
    )
      .then((response) => {
                      ...
                      <tr>
                        <td colSpan="8">
                          <div className="additional-info">
                            ...
                            <div className="attribute">
                              <strong>
                                ¿Es necesario realizarse una perno+corona?
                              </strong>{" "}
                              {
                                item.procedimientos?.M?.protesis?.M
                                  ?.protesis_perno_corona?.S
                              }
                            </div>
                          </div>
                        </td>
